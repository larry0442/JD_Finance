...
<section :class="$style.content">
  <div :class="$style.loginPage">
    <!-- 短信登录 -->
    <div :class="$style.smsLogin">
      <!-- 输入手机号码 -->
      <div :class="$style.inputBox">
        <div :class="$style.inputContainer">
          <label :class="$style.areaBox">+86</label>
          <input type="tel" :class="$style.tel" placeholder="请输入手机号"/>
        </div>
        <button :class="$style.btn">获取验证码</button>
      </div>
      <!-- 验证码 -->
      <div :class="$style.code">
        <input type="tel" :class="$style.check" placeholder="请输入收到的验证码"/>
      </div>
    </div>
    <!-- 分隔开短信验证和登录按钮 -->
    <div :class="$style.notice">&nbsp;</div>
    <router-link :to="{ name: 'home'}">
      <span :class="$style.btn1">登&nbsp;录</span>
    </router-link>
    <router-link :to="{ name: 'home'}">
      <span :class="$style.btn2">一键登录</span>
    </router-link>
    ...
    <div :class="$style.loginType">
      <div :class="$style.quickLogin">
        <h4>其他登录方式</h4>
        <router-link :to="{ name: 'home'}">
          <i :class="$style.iconqq"></i>
          <br />
          <span :class="$style.txt">QQ</span>
        </router-link>
        <router-link :to="{ name: 'home'}">
          <i :class="$style.iconwechat"></i>
          <br />
          <span :class="$style.txt">微信</span>
        </router-link>
      </div>
    </div>
    <!--协议  -->
    <div :class="$style.agreementTips">
        <p>登录即代表您已同意<a href="#home">京东隐私政策</a></p>
    </div>
  </div>
</section>
</template>
...
.content{
  ...
  .loginPage{
    ...
    .btn1{
      ...
      box-shadow: 0 2px 4px 0 rgba(255,62,62,.2);
      text-align: center;
    }
    .btn2{
      width: 100%;
